#ifndef __LETRA_H // Essas diretivas são usadas para garantir que o arquivo de cabeçalho data.h... 
#define __LETRA_H // não seja incluído mais de uma vez em um mesmo arquivo de código (prevenindo a inclusão múltipla)

// LETRA representa uma imagem de 28x28 pixels, que representa uma letra do alfabeto.
...
class data
{
  //E sse vetor vai guardar os pixels da imagem.
  vector<uint8_t> *vec_de_pixels;   // definição do ponteiro  "vec_de_pixels" que aponta para um "vector<uint8_t>".
  ...
  void def_pont_para_vec_de_pixels(vector<uint8_t> *); // Atribui o ponteiro "vetor_de_recurso" para apontar para um vetor passado...
                                                    // como argumento. 
  void incluir_no_vec_de_pixels   (uint8_t);           // colocar um elemento no vetor de recursos
  ...
  int     obter_tamanho_do_vec_de_pixels();      
  uint8_t obter_label(); 
  uint8_t obter_label_enumerada();

  vector<uint8_t> * obter_vec_de_pixels(); // Esse método retorna o ponteiro para o vetor de recursos, ...
